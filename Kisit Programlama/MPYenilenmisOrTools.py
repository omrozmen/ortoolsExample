...
def OrtoolsSolver(data, points):
    """OR-Tools ile TSP'yi çöz ve rota ve maliyeti döndür."""
    ...
    searchParameters.time_limit.seconds = 5  # Set a shorter time limit for optimization
...
def ShortestRoute(pointsList, data):
    """Birden fazla set arasında en kısa rotayı OR-Tools ile bulun."""
    minRoute = None
    minCost = float('inf')
    
    for points in pointsList:
        route, cost = OrtoolsSolver(data, points)
    ...
    print("\nEn Kısa Rota Sonucu:")
    if minRoute:
        print("En Kısa Rota:", [istanbulGezi[point] for point in minRoute])
