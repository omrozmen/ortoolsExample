...
def measure_time_of_model():
    start_time = time.time()
    # Modelin çözümünü çağıran fonksiyon burada yer almalı, örneğin:
    # status = solve_model(model)
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f'Model çözüm süresi: {elapsed_time:.4f} saniye')

# Bu fonksiyonu mevcut kodun içinde uygun yerde çağırın.
