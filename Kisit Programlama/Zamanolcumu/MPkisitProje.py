...
def GurobiTsp(points):
    ...
    model.setObjective(gp.quicksum(distance_matrix[points[i]][points[j]] * x[i, j] for i in range(n) for j in range(n)), GRB.MINIMIZE)
    ...
    if model.status == GRB.OPTIMAL:
        ...
        while len(route) < n:
            ...
            for j in range(n):
                if solution[next_city, j] > 0.5:
                    next_city = j
                    break
        return route, model.objVal
    else:
...
def main():
    for idx, points in enumerate(pointsList):
        print('\nRota {}:'.format(idx + 1))
        route, cost = GurobiTsp(points)
        selected_locations = [istanbulGezi[point] for point in points]
        if route:
            route_locations = [istanbulGezi[points[point]] for point in route] + [istanbulGezi[points[route[0]]]]
    ...
    if min_route:
        min_route_locations = [istanbulGezi[min_points[point]] for point in min_route] + [istanbulGezi[min_points[min_route[0]]]]
        print("\nEn Kısa Rota Sonucu:")
        print("En Kısa Rota:", min_route_locations)
        print("En Kısa Rota Maliyeti: {} metre".format(min_cost))
