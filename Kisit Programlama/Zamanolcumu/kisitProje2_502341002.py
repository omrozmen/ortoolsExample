...
def OrtoolsSolver(points):
    ...
    if solution:
        ...
        route.append(points[manager.IndexToNode(index)])
        return route, routeCost, originalMatrix
    else:
...
def ShortestRoute(pointsList):
    ...
    for points in pointsList:
        route, cost, data = OrtoolsSolver(points)
    ...
    print("\nEn Kısa Rota Sonucu:")
    if minRoute:
        minRoute_locations = [istanbulGezi[point] for point in minRoute]
        print("En Kısa Rota:", minRoute_locations)
        print("En Kısa Rota Maliyeti: {} metre".format(minCost))

        # En kısa rotaya ait noktaları yazdırma
...
def main():
    ...
    for idx, points in enumerate(pointsList):
        print('\nRota {}:'.format(idx + 1))
        route, cost, originalMatrix = OrtoolsSolver(points)
        selected_locations = [istanbulGezi[point] for point in points]
        route_locations = [istanbulGezi[point] for point in route]
        print("Seçilen Noktalar:", selected_locations)
        if route:
            print("Rota:", route_locations)
            ...
            for i, point in enumerate(route[:-1]):
                ...
                print(f"{istanbulGezi[point]} -> {istanbulGezi[next_point]}: {originalMatrix[point][next_point]} metre")
