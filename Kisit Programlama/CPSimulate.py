...
istanbulGeziKoordinatlar = {
    "Sultanahmet Camii": (41.0082, 28.9784),
    "Topkapı Sarayı": (41.0115, 28.9831),
    "Ayasofya Müzesi": (41.0086, 28.9802),
    "Kapalıçarşı": (41.0106, 28.9680),
    "Galata Kulesi": (41.0257, 28.9744),
    "İstiklal Caddesi": (41.0348, 28.9838),
    "Taksim Meydanı": (41.0369, 28.9851),
    "Dolmabahçe Sarayı": (41.0391, 29.0009),
    "Yerebatan Sarnıcı": (41.0086, 28.9770),
    "Pierre Loti Tepesi": (41.0531, 28.9471),
    "Rahmi Koç Müzesi": (41.0424, 28.9496),
    "Miniatürk": (41.0612, 28.9486),
    "Emirgan Korusu": (41.1096, 29.0551),
    "Rumeli Hisarı": (41.0875, 29.0567),
    "Kız Kulesi": (41.0210, 29.0044),
    "Beylerbeyi Sarayı": (41.0421, 29.0402),
    "Çamlıca Tepesi": (41.0124, 29.0694),
    "Adalar": (40.8761, 29.0983),
    "Kadıköy": (40.9916, 29.0268),
    "Moda Sahil Parkı": (40.9826, 29.0344)
}
...
def OrtoolsSolver(data, points):
    subMatrix = [[data['distance_matrix'][i][j] for j in points] for i in points]

    manager = pywrapcp.RoutingIndexManager(len(subMatrix), data['num_vehicles'], data['depot'])
...
def plot_route_on_map(route):
    ...
    route_coordinates = [istanbulGeziKoordinatlar[istanbulGezi[point]] for point in route]
    
    folium.PolyLine(route_coordinates, color="blue", weight=2.5, opacity=1).add_to(route_map)
    
    for point in route:
        folium.Marker(
            location=istanbulGeziKoordinatlar[istanbulGezi[point]],
            popup=istanbulGezi[point],
...
route, cost = OrtoolsSolver(data, pointsList[0])
print("Rota:", [istanbulGezi[point] for point in route])
print("Toplam maliyet: {} metre".format(cost))

route_map = plot_route_on_map(route)
route_map.save("istanbul_shortest_route.html")
